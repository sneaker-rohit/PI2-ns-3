...
NS_LOG_COMPONENT_DEFINE ("PieSecond");
...
int main (int argc, char *argv[])
{
  ...
  std::string pcapFileName = "pie-second.pcap";
  ...
  if (writeForPlot)
    {
      filePlotQueueDel << pathOut << "/" << "pie-second-delay.plotme";
  ...
  flowmon.SerializeToXmlFile ("pie-second.xml", true, true);
  ...
  if (printPiStats)
    {
      ...
      std::cout << "*** PIE stats from Node 50 queue ***" << std::endl;
      ...
      std::cout << "*** PIE stats from Node 51 queue ***" << std::endl;
      std::cout << "\t " << st.unforcedDrop << " drops due to probability " << std::endl;
      std::cout << "\t " << st.forcedDrop << " drops due queue full" << std::endl;
    }
