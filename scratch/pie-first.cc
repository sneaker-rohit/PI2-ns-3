...
NS_LOG_COMPONENT_DEFINE ("PieFirst");
...
int main (int argc, char *argv[])
{
  ...
  std::string pcapFileName = "pie-first.pcap";
  ...
  if (writeForPlot)
    {
      filePlotQueueDel << pathOut << "/" << "pie-first-delay.plotme";
  ...
  flowmon.SerializeToXmlFile ("pie-first.xml", true, true);
  ...
  if (printPieStats)
    {
      ...
      std::cout << "*** PIE stats from Node 5 queue ***" << std::endl;
      ...
      std::cout << "*** PIE stats from Node 6 queue ***" << std::endl;
      std::cout << "\t " << st.unforcedDrop << " drops due to probability " << std::endl;
      std::cout << "\t " << st.forcedDrop << " drops due queue full" << std::endl;
    }
