...
NS_LOG_COMPONENT_DEFINE ("PiSquareFirst");
...
{
  bool printPiSquareStats = true;
  bool  isPcapEnabled = true;
  float startTime = 0.0;
  float simDuration = 100;      // in seconds
  std::string  pathOut = ".";
  bool writeForPlot = true;
  std::string pcapFileName = "pi-square-first.pcap";
  ...
  tchPiSquare.SetRootQueueDisc ("ns3::PiSquareQueueDisc");
  ...
  QueueDiscContainer queueDiscs = tchPiSquare.Install (devices_gateway);
  ...
  if (writeForPlot)
    {
      filePlotQueueDel << pathOut << "/" << "pi-square-first-delay.plotme";
  ...
  flowmon.SerializeToXmlFile ("pi-square-first.xml", true, true);

  Simulator::Stop (Seconds (stopTime));
  Simulator::Run ();

  if (printPiSquareStats)
    {
      PiSquareQueueDisc::Stats st = StaticCast<PiSquareQueueDisc> (queueDiscs.Get (0))->GetStats ();
      std::cout << "*** PI Square stats from Node 5 queue ***" << std::endl;
      ...
      std::cout << "*** PI Square stats from Node 6 queue ***" << std::endl;
      std::cout << "\t " << st.unforcedDrop << " drops due to probability " << std::endl;
      std::cout << "\t " << st.forcedDrop << " drops due queue full" << std::endl;
    }
