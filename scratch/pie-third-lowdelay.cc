...
NS_LOG_COMPONENT_DEFINE ("PieThird");
...
int main (int argc, char *argv[])
{
  ...
  std::string pcapFileName = "pie-third-lowdelay.pcap";
  ...
  Config::SetDefault ("ns3::PieQueueDisc::QueueDelayReference", TimeValue (Seconds (0.005)));
  ...
  if (writeForPlot)
    {
      filePlotQueueDel << pathOut << "/" << "pie-third-lowdelay.plotme";
  ...
  flowmon.SerializeToXmlFile ("pie-third-lowdelay.xml", true, true);
  ...
  if (printPiStats)
    {
      ...
      std::cout << "*** PIE stats from Node 7 queue ***" << std::endl;
      std::cout << "\t " << st1.unforcedDrop << " drops due to probability " << std::endl;
      std::cout << "\t " << st1.forcedDrop << " drops due queue full" << std::endl;

      PieQueueDisc::Stats st2 = StaticCast<PieQueueDisc> (queueDiscs.Get (1))->GetStats ();
      std::cout << "*** PIE stats from Node 8 queue ***" << std::endl;
