/*
 * This script simulates 20 TCP flows for PI Square evaluation
...
NS_LOG_COMPONENT_DEFINE ("PiSquareFourth");
...
int main (int argc, char *argv[])
{
  ...
  std::string pcapFileName = "pi-square-fourth.pcap";
  ...
  if (writeForPlot)
    {
      filePlotQueueDel << pathOut << "/" << "pi-square-fourth-delay.plotme";
  ...
  flowmon.SerializeToXmlFile ("pi-square-fourth.xml", true, true);
  ...
  if (printPiStats)
    {
      ...
      std::cout << "*** PI Square stats from Node 20 queue ***" << std::endl;
      ...
      std::cout << "*** PI Square stats from Node 21 queue ***" << std::endl;
      std::cout << "\t " << st.unforcedDrop << " drops due to probability " << std::endl;
      std::cout << "\t " << st.forcedDrop << " drops due queue full" << std::endl;
    }
